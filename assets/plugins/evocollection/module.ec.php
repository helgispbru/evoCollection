...
if (file_exists(MODX_BASE_PATH . "assets/plugins/evocollection/lang/" . $modx->config['manager_language'] . ".php")) {
    require_once MODX_BASE_PATH . "assets/plugins/evocollection/lang/" . $modx->config['manager_language'] . ".php";
} else {
    require_once MODX_BASE_PATH . "assets/plugins/evocollection/lang/russian-UTF8.php";
}
    ...
    foreach ($_POST as $key => $val) {
        if (is_array($val)) {
            foreach ($val as $k1 => $v1) {
                $configuration[$key][$k1] = $modx->db->escape($v1);
            }
        } else {
            $configuration[$key] = $modx->db->escape($val);
        }
    }
    $configuration = $_POST;
...
if (($idc) or ($action == 'new')) {
    if ($idc) {
        $pagetitle = $lang['edit_collection'];
    } else {
        $pagetitle = $lang['new_collection'];
    }
    ...
    $module .= '>
        ...
        <option value="0"';
    if ($config[$idc]['show_child'] == '0') {
        $module .= ' selected="selected" ';
    }

    $module .= '>' . $lang['no'] . '</option>
        <option value="1"';
    if ($config[$idc]['show_child'] == '1') {
        $module .= ' selected="selected" ';
    }
    ...
    foreach ($optArr as $key => $val) {
        $module .= '<option value="' . $key . '">' . $val . '</option>';
    }
    ...
    $module .= '</tbody>
        ...
        <option value="asc"';
    if ($config[$idc]['direction'] == 'asc') {
        $module .= ' selected="selected" ';
    }

    $module .= '>' . $lang['asc'] . '</option>
        <option value="desc"';
    if ($config[$idc]['direction'] == 'desc') {
        $module .= ' selected="selected" ';
    }
    ...
    while ($row = $modx->db->getRow($templates)) {
        ...
        if ($row['id'] == $config[$idc]['template_default']) {
            $ss = ' selected="selected"';
        } else {
            $ss = '';
        }
    ...
    if ($config[$idc]['how_edit'] == 'dblclick') {
        $module .= ' selected="selected" ';
    }

    $module .= '>' . $lang['dblclick'] . '</option>
        <option value="contextmenu"';
    if ($config[$idc]['how_edit'] == 'contextmenu') {
        $module .= ' selected="selected" ';
    }
