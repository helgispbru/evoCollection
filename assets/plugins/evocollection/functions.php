...
if (!function_exists('get_output')) {
    function get_output($config)
    {
        global $modx;
        if ($config['user_func']) {
            return $modx->runSnippet($config['user_func'], $config);
        }
        ...
        switch ($mode) {
            case 'output':
                switch ($type) {
                    ...
                    case 'image':
                        if ((file_exists(MODX_BASE_PATH . $value)) && ($value)) {
                            ...
                            if (file_exists(MODX_BASE_PATH . 'assets/plugins/evocollection/cache/' . $value)) {
                                ...
                                return '<i class="fa fa-spinner fa-spin noimgs" data-href="' . $value . '"></i>';
                            }
                        } else {
                            return '<img src="./../assets/snippets/phpthumb/noimage.png" width="64" height="64">';
                        }
                    ...
                    case 'oncecheckbox':
                        if ($value) {
                            return '<div style="text-align:center;">Да</div>';
                        } else {
                ...
                if ((!$return) && (!$value)) {
                    $return = '<div class="extender">не задан</div>';
                }
                ...
                switch ($type) {
                    ...
                    case 'select':
                        ...
                        foreach (explode($delimiter, $elements) as $str) {
                            ...
                            if (isset($col[1])) {
                                if ($col[1] == $value) {
                                    $s .= '<option value="' . $col[1] . '" selected="selected">' . $col[0] . '</option>';
                                } else {
                                    $s .= '<option value="' . $col[1] . '">' . $col[0] . '</option>';
                                }
                            } else {
                                if ($col[0] == $value) {
                                    $s .= '<option value="' . $col[0] . '" selected="selected">' . $col[0] . '</option>';
                                } else {
                                    $s .= '<option value="' . $col[0] . '">' . $col[0] . '</option>';
                                }
                            }
                        }
                        return $s . '</select>';
